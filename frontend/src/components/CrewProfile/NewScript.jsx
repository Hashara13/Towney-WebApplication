...
const NewScriptPost = () => {
  ...
  const removeCrewPosition = (index) => {
    if (scriptData.crewPositions.length > 1) {
      setScriptData(prevData => ({
        ...prevData,
  ...
  const handleSubmit = async (e) => {
    ...
    const isValidCrewPositions = scriptData.crewPositions.every(position => position.role.trim() !== '');
    if (!isValidCrewPositions) {
      setError('All crew position roles must be filled.');
      return;
    }

    try {
      const response = await axios.post('http://localhost:5000/script/new', {
        ...scriptData,
        budget: parseFloat(scriptData.budget),
        // postedBy: "67110589a1308e46c524e206"
      });
      console.log('Script posted successfully:', response.data);
      navigate('/scripts');
    } catch (error) {
      console.error('Error posting script:', error.response?.data || error.message);
      setError('Failed to post script. Please try again.');
    }
  };
